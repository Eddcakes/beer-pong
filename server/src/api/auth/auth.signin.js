...
const selectUserLogin = `SELECT users.id, users.email, users.password, users.username, users.active, users.role FROM ${process.env.DATABASE}.users WHERE LOWER(users.username) = LOWER($1) AND active = true`;
